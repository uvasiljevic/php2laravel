                            ...
                            <ul>
                                @foreach($menu as $m)
                                    @isset($m->categories)
                                        ...
                                        @endif
                                    @endisset
                                    <a href="{{url("/$m->link")}}">{{$m->title}}</a>
                                    @isset($m->categories)
                                        @if(count($m->categories)>0)
                                        <ul>
                                            @foreach($m->categories as $mc)
                                                <li><a href="{{url("/products/category/$mc->id")}}">{{$mc->name}}</a></li>
                                            @endforeach
                                        </ul>
                                        @endif
                                    @endisset
        ...
        <div class="page_menu_content">
            ...
            <ul class="page_menu_nav menu_mm">
                    ...
                    @foreach($menu as $m)
                        ...
                        @endif
                        @endisset
                            <a href="{{url("/$m->link")}}">{{$m->title}}<i class="fa fa-angle-down"></i></a>
                            @isset($m->categories)
                                @if(count($m->categories)>0)
                                    <ul class="page_menu_selection menu_mm">
                                        @foreach($m->categories as $mc)
                                            <li class="page_menu_item menu_mm"><a href="{{url("/products/category/$mc->id")}}">{{$mc->name}}</a></li>
                                        @endforeach
                                    </ul>
                                @endif
                            @endisset
